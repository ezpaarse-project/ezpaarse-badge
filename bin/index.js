...
const run = async () => {
  ...
  request(`${urlApi}/client/OBF.rsa.pub`, (error, response, body) => {
    ...
    exec(`openssl req -new -nodes -batch -days 1095 -newkey rsa:2048 -keyout ./app/ssl/obf.key -subj '/CN=${decrypt.id}' > ./app/ssl/obf.csr`, (err, stdout, stderr) => {
      ...
      }, (errorCsr, responseCrs, bodyCsr) => {
        if (errorCsr || bodyCsr.error) {
          // eslint-disable-next-line
          console.log(chalk.red(errorCsr || bodyCsr.error))
          process.exit(1);
        }

        fs.writeFileSync('./app/ssl/certificate.pem', bodyCsr);
        request({
          method: 'GET',
          url: `${urlApi}/ping/${decrypt.id}`,
        }, (errorPem, responsePem, bodyPPem) => {
          // eslint-disable-next-line
          if (errorPem || bodyPPem.error) console.log(chalk.red(errorPem || bodyPPem.error))
